...
const DrawerContent = () => (
  <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
    ...
    <List sx={{ flex: 1, pt: 2 }}>
      {MENU_ITEMS.map(item => (
        <ListItem key={item.id} button sx={{ mx: 1, mb: 0.5, borderRadius: '8px', '&:hover': { backgroundColor: '#f5f5f5' }, ...(item.id === 'users' && { backgroundColor: '#f5f5f5', '& .MuiListItemIcon-root': { color: '#aa3bff' }, '& .MuiListItemText-primary': { color: '#aa3bff', fontWeight: 600 } }) }}>
    ...
    <Box sx={{ p: 2 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        <Avatar sx={{ width: 36, height: 36, background: 'linear-gradient(135deg, #aa3bff 0%, #7a1fa3 100%)', fontSize: '18px' }}>👨</Avatar>
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography variant="body2" sx={{ fontWeight: 600, color: '#08060d' }} noWrap>Admin User</Typography>
          <Typography variant="caption" sx={{ color: '#6b6375' }} noWrap>admin@petcare.com</Typography>
        </Box>
      </Box>
    </Box>
  </Box>
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#f9f9f9' }}>
      <AppBar position="fixed" sx={{ backgroundColor: '#ffffff', color: '#08060d', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', zIndex: theme => theme.zIndex.drawer + 1 }}>
        <Toolbar sx={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Avatar onClick={e => setAnchorEl(e.currentTarget)} sx={{ cursor: 'pointer', background: 'linear-gradient(135deg, #aa3bff 0%, #7a1fa3 100%)', width: 36, height: 36, fontSize: '18px' }}>👨</Avatar>
            <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} transformOrigin={{ vertical: 'top', horizontal: 'right' }}>
              ...
              <Divider />
              <MenuItem onClick={() => setAnchorEl(null)}><Logout sx={{ mr: 1, fontSize: 20 }} />Logout</MenuItem>
            </Menu>
          </Box>
        </Toolbar>
      </AppBar>
      ...
      <Box component="main" sx={{ flex: 1, display: 'flex', flexDirection: 'column', marginTop: '64px', minHeight: 'calc(100vh - 64px)' }}>
        <Box sx={{ flex: 1, p: { xs: 2, md: 3 }, overflowY: 'auto' }}>{children}</Box>
